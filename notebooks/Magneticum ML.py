...
# <span style="color:red">When finalized, generate two separate Jupyter notebooks for the Chandra and eROSITA datasets in order to upload to Github.</span>
...
def total_counts(cluster_id):
    # returns photon count within R500
    dat = fits.open(cluster_directory / ('%d.fits' % cluster_id))[0].data
    r500 = magneticum_data['R500_pixel'].loc[cluster_id]
    total_mask = (cell_dists <= r500)
    Ntot = np.sum(dat[total_mask])
...
# we will actually do the analysis that is present below in a section further down where we have our final cluster sample
# so that we can report our results as computed on the final cluster sample


# In[38]:


t_counts = np.zeros(Nclusters)
for i,cluster_id in enumerate(magneticum_data.index.values):
    if(i % 100 == 0):
        print(i)
    t_counts[i] = total_counts(cluster_id)


# In[39]:


# histogram of counts vs redshift
redshifts = np.unique(magneticum_data['redshift'])
cols = sns.cubehelix_palette(len(redshifts))
for i in range(0,len(redshifts)):
    plt.hist(np.log10(t_counts[magneticum_data['redshift'] == redshifts[i]]), alpha=0.5, color=cols[i], label=redshifts[i])
plt.legend()

print(np.median(t_counts[magneticum_data['redshift'] == redshifts[0]]))
print(np.median(t_counts[magneticum_data['redshift'] == redshifts[-1]]))
#1874.5 and 103.0 for eROSITA
#308546.0 and 31564.0 for Chandra
...
#let's just look at total counts within R500c and get a distribution so we can quote something; doing this for eROSITA only
...
# make a distribution of redshifts to address Lorenzo's question
plt.hist(magneticum_data['redshift'])
print(np.unique(magneticum_data['redshift'],return_counts=True))


# In[11]:


t_counts = np.zeros(len(masses))
for i,cluster_id in enumerate(magneticum_data.index.values):
    if(i % 100 == 0):
        print(i)
    t_counts[i] = total_counts(cluster_id)
...
print(np.median(t_counts[magneticum_data['redshift'] == redshifts[0]]))
print(np.median(t_counts[magneticum_data['redshift'] == redshifts[-1]]))
# Chandra: 555042.5 and 29673.0
# eROSITA: 1874.5 and 93.0
...
# ### Deciding if any clusters are larger than Chandra aperture

# In[ ]:


# we're going to flag these and throw them out, then run the RF model and see how things change
# no need to re-cross-validate using this dataset, just use the same best set of hyperparameters
# if the scatter is very similar, then we will report this and move on


# In[16]:


# angles in radians: R500 / angular diameter distance
angs_rad = 1e-3 * magneticum_data['r500_kpch'] / (magneticum_data['Dcomoving_Mpch'] / (1. + magneticum_data['redshift']))
angs_deg = angs_rad * 360. / (2.*np.pi) # for the radius, in degrees
print(np.max(angs_deg))
chandra_detector = 0.2816667 #degrees
# Chandra detector is 16.9 arcmin = 0.2816667 degrees
# largest cluster has radius of 0.23282 degrees, so diameter is larger than the Chandra detector
# How many clusters, and which ones, are larger than the Chandra detector
magneticum_data[angs_deg*2 > chandra_detector]['M500_msolh']


# In[20]:


plt.hist(2*angs_deg[angs_deg*2 > chandra_detector] / chandra_detector)


# In[18]:


# drop the bad clusters
magneticum_data = magneticum_data[angs_deg*2 <= chandra_detector]
print(magneticum_data.shape)
...
if(compare_dists):
    ...
    plt.rc('text', usetex=False)
    magneticum_data.drop(columns=['logM500','logLx_ex','uid']).hist(bins=50, figsize=(20,20), histtype="step",color='k', linewidth=1.2);
...
# look et redshift evolution of the mp+Lx pdfs to justify
unique_z = np.unique(mag_redshifts)
cols = sns.cubehelix_palette(len(unique_z))
plt.rc('text', usetex=False)
ax = magneticum_data.drop(columns=['logM500','uid'])[mag_redshifts == unique_z[0]].hist(bins=50, figsize=(20,20), histtype="step",color=cols[0], linewidth=1.2, label=r'$z=$%.2f' % unique_z[0], density='normed');
for i in range(1, len(unique_z)):
    magneticum_data.drop(columns=['logM500','uid'])[mag_redshifts == unique_z[i]].hist(bins=50, figsize=(20,20), histtype="step",color=cols[i], linewidth=1.2, ax=ax.reshape((12,-1))[0:12], label=r'$z=$%.2f' % unique_z[i], density='normed');
...
# compute the mean/std dev of each morph param in each redshift and see how much they differ from their mean
for cat in cats_to_keep:
    means = np.zeros(len(unique_z))
    for i in range(0, len(unique_z)):
        means[i] = np.mean(magneticum_data[cat][mag_redshifts == unique_z[i]])
    mean_of_means = np.mean(means)
    # see what percentage the distribution means vary from the mean over all redshifts
    differences = ((means/mean_of_means)-1)*100
    print(cat,differences)
    
    # we can quantify by saying for the mps+Lx, the individual redshift distributions mean/std devs vary by less than
    # x% relative to the full distribution


# In[20]:


# generate figure for paper that plots cAs, caS, c
edges=[(0,1), (0.8,2.2), (0,1.8)]
if(compare_dists):
    plt.rc('text', usetex=True)
    fig, ax = plt.subplots(1,3, figsize=(14,3))
    cats = ['concentration', 'cAs', 'caS']
    #magneticum_data[['concentration', 'cAs', 'caS']].hist(bins=50, figsize=(20,20), histtype="step",color='k', linewidth=1.2, label='Chandra', density='normed', ax=ax);
    #morph_parms_erosita[['concentration', 'cAs', 'caS']].hist(bins=50, histtype="step",color='r', linewidth=1.2, ax=ax, label='eROSITA', density='normed');
    for i in range(0,3):
        ax[i].set_ylabel(r'PDF')
        #ax[i].set_title('')
        ax[i].hist(magneticum_data[cats[i]],bins=50,histtype="step", color="k", linewidth=1.2, label='Chandra', density='normed', range=edges[i])
        ax[i].hist(morph_parms_erosita[cats[i]],bins=50,histtype="step", color="r", linestyle='dashdot', linewidth=1.2, label='eROSITA', density='normed', range=edges[i])
    ax[0].set_xlabel(r'$c$')
    #ax[0].set_xlim(0,3)
    ax[1].set_xlabel(r'$A$')
    ax[2].set_xlabel(r'$S$')
    ax[0].legend(loc=1, frameon=False)
    plt.savefig(fig_dir / 'feature_dists_cas.pdf',bbox_inches='tight')
    plt.show()
...
np.savez(data_dir / 'chandra_training_data_and_folds_cas',logo_iter=logo_iter, train=train, train_labels=train_labels)
...
cut = np.log10(h*3e14)
#cut = np.log10(8e13)
train_msk = train_labels >= cut
test_msk  = test_labels >= cut
...
lin_reg = LinearRegression()
lin_reg.fit(train[:,train_columns == 'logLx_ex'].reshape(-1,1), train_labels)
test_predictions = lin_reg.predict(test[test_msk,train_columns == 'logLx_ex'].reshape(-1,1))
train_predictions = lin_reg.predict(train[train_msk, train_columns == 'logLx_ex'].reshape(-1,1))
m_lx_scatter_robust, m_lx_sc_rb_2sig = scatter_prelog(test_predictions, test_labels[test_msk])
m_lx_scatter_std = scatter_prelog(test_predictions, test_labels[test_msk], False)
...
lin_reg = LinearRegression()
lin_reg.fit(train[train_msk,train_columns == 'logLx_ex'].reshape(-1,1), train_labels[train_msk])
test_predictions = lin_reg.predict(test[test_msk,train_columns == 'logLx_ex'].reshape(-1,1))
train_predictions = lin_reg.predict(train[train_msk, train_columns == 'logLx_ex'].reshape(-1,1))
m_lx_scatter_robust, m_lx_sc_rb_2sig = scatter_prelog(test_predictions, test_labels[test_msk])
m_lx_scatter_std = scatter_prelog(test_predictions, test_labels[test_msk], False)
...
# the scatter is actually 17% compared to the 15% scatter seen in the Mantz paper when we actually use the same lower
# bound on the mass

# if we use the mass cutoff from Maughan, roughly 8e13, then we get 21.6% scatter, so we're in pretty reasonable territory
...
robust_bias = np.median(test_predictions - test_labels)
bias = np.mean(test_predictions - test_labels)
print('The M-Lx model applied to test set had a (robust) bias of %.4f dex' % robust_bias)
print('The M-Lx model applied to test set had a bias of %.4f dex' % bias)
...
#Using our RF model with all features:
#The (robust) scatter for a RF model is 0.06613 dex
#Robust 2-sigma scatter: 0.157 dex
#The (std) scatter for a RF model is 0.08511 dex

#Using our RF model with only L_x, c, A, S: (worth noting that this may be worth cross-validating)
#The (robust) scatter for a RF model is 0.06772 dex
#Robust 2-sigma scatter: 0.167 dex
#The (std) scatter for a RF model is 0.08874 dex


# In[20]:


#erosita cas CV: {'bootstrap': True, 'max_depth': 80, 'max_features': 0.5, 'min_samples_leaf': 1, 'min_samples_split': 5, 'n_estimators': 1800}
#chandra cas CV: {'bootstrap': True, 'max_depth': 30, 'max_features': 0.5, 'min_samples_leaf': 1, 'min_samples_split': 3, 'n_estimators': 1000}
rf = RandomForestRegressor(random_state=42, bootstrap=True, max_depth=30, max_features=0.5, min_samples_leaf=1,
...
#The (robust) scatter for a RF model is 0.07013 dex
#Robust 2-sigma scatter: 0.162 dex
#The (std) scatter for a RF model is 0.08708 dex
...
# how about below the 10^14.4 case?
test_cut = test_labels <= 14.4
rf_scatter_robust, rf_sc_rb_2sig = scatter_prelog(rf_preds[test_cut], test_labels[test_cut])
rf_scatter_std = scatter_prelog(rf_preds[test_cut], test_labels[test_cut], False)
print("The (robust) scatter for a RF model is %.5f dex" % rf_scatter_robust)
print("Robust 2-sigma scatter: %.3f dex" % rf_sc_rb_2sig)
print("The (std) scatter for a RF model is %.5f dex" % rf_scatter_std)
...
# lorenzo wants to see results plotted relative to median of distribution
print(np.median(test_labels)) # median is nearly 1e14
...
# add in error bars on the line using the robust scatter and overplot crosses on the clusters
# that extend beyond the aperture, making a note of it in both the text and the caption

log10_mass_median = 14 #log10 of 1e14 Msun/h

msk = np.isin(test_set.index, magneticum_data[angs_deg*2 > chandra_detector].index)
# requires that you run the code to identify cases where radius is larger than Chandra FoV

fig, ax = plot()
plt.plot(test_labels[~msk], rf_preds[~msk], '.', markersize=8, alpha=0.75)
plt.plot(test_labels[msk], rf_preds[msk], '.', color='r', markersize=8, alpha=0.75)
...
plt.text(14.2,13.8,r'Idealized \textit{Chandra}', fontsize=18)
...
plt.savefig(fig_dir / 'mpred_vs_mtrue_chandra.pdf', bbox_inches='tight')
...
plt.hist(rf_preds - test_labels, histtype='step', color='k', linestyle='solid',
         linewidth=2, bins=30, density="normed", label=r'\noindent RF \textit{Chandra} \vspace{1.5mm} \\  %.3f dex, \vspace{1.5mm} \\ %.2f percent' %(rf_scatter_robust,(10**rf_scatter_robust - 1)*100)) #change this to the actual %age
plt.hist(rf_preds_systematic - test_labels, histtype='step', color='r', linestyle='dashdot',
         linewidth=2, bins=30, density="normed", label=r'\noindent RF \textit{eROSITA} \vspace{1.5mm} \\  %.3f dex, \vspace{1.5mm} \\ %.2f percent' %(rf_scatter_systematic_robust,(10**rf_scatter_systematic_robust - 1)*100))
plt.hist(mlx_predictions - test_labels, histtype='step', color='b', linestyle='dashed',
         linewidth=2, bins=30, density="normed", label=r'\noindent $M-L_{\mathrm{ex,z}}$  \vspace{1.5mm} \\  %.3f dex, \vspace{1.5mm} \\ %.2f percent' %(m_lx_scatter_robust,(10**m_lx_scatter_robust - 1)*100))
plt.xlabel(r'$\log(M_{500c,\mathrm{pred}}) - \log(M_{500c,\mathrm{true}})$')
plt.ylabel('PDF');

handles, labels = ax.get_legend_handles_labels()
handles.insert(2,mlines.Line2D([],[],linestyle=''))
labels.insert(2,'')
handles.insert(1,mlines.Line2D([],[],linestyle=''))
labels.insert(1,'')
handles.insert(0,mlines.Line2D([],[],linestyle=''))
labels.insert(0,'')
...
cluster_id = 871048
gen_plot(cluster_id, vmin=0, vmax=1.7)
